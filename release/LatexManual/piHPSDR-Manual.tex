...
\texttt{LINUX/pulse.vac.sh} in which the two commands above are contained.
...
Note that these devices only exist until the computer is shut down,
which means that the commands given above have to be repeated after rebooting
the machine (or restarting PulseAudio). They can be made permanent (you will find
...
\vspace{0.1cm}
\grtt{pacmd set-default-sink <index of TXcable>}

\vspace{0.1cm}
\grtt{pacmd set-default-source <index of RXcable.monitor>}
...
done in the same way as described in the previous chapter, but using
 \texttt{pactl} command instead of \texttt{pacmd}.
...
A shell script \texttt{LINUX/pipewire.sh} is provided that contains these two commands.
