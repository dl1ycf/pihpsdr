...
\item{NR3 (RNNnoise) and NR4 (SpecBleach) noise reduction models fully integrated.}
...
\item{\pH 2.6 will be released when this page is filled.}
\end{itemize}
\clearpage
...
\item[G2V1 Front Panel]{Choose this if you have a first-generation (7-inch screen) Anan G2
 radio with a built-in controller.}
\end{itemize}
...
For an Anan G2 radio, only the first item (which then reads \rett{G2V2}) and the last item are available.
For second-generation G2 radios (8-inch screen with LEDs) the \rett{G2V2} option (which is identical
to \rett{No Controller}) has to be chosen since for these radios, the front panel is not
controlled via GPIO. To avoid confusion on the user's side, \pH on second-generation G2 radios
should be compiled without GPIO support.
}
}
\end{center}
...
If using an external controller (GPIO, MIDI, G2 V1 or V2 front panel, ANDROMEDA), one can
...
radio.
The following figure (Fig. \ref{fig:RadioMenu}) shows the Radio menu. In the top right corner
there are radio specific settings that are discussed below. First we go through all the elements we see in Fig. \ref{fig:RadioMenu}.
...
\rett{Sample Rate}. This box does not appear for HPSDR radios running protocol-2, since there each receiver
...
\rett{3dB/S-mtr step}. There is an IARU recommendation that 1 step on the S-meter
...
The next line (below the horizontal line) with check boxes \rett{Split}, \rett{Duplex},
\rett{Mute RX on TX} and \rett{PA enable} is only shown for radios that have a transmitter.
...
only appears for HPSDR (and not for SoapySDR) radios, since it assigns actions to
...
\subsubsection{Orion/Orion-II/Saturn}
For these radios, the upper right corner of the radio menu is as shown in
Fig. \ref{fig:RadioMenuG2}:

\begin{figure}[ht]
\center
\includegraphics[scale=0.6]{RadioMenuG2.png}
\caption{\bltt{Radio} menu settings for Orion/OrionII/Saturn}
\label{fig:RadioMenuG2}
\end{figure}
...
The settings for an ATLAS (first-generation HPSDR) system are shown in Fig. \ref{fig:RadioMenuMetis}.
...
cards that can be plugged into a bus system.

\begin{figure}[ht]
\center
\includegraphics[scale=0.6]{RadioMenuMetis.png}
...
The HermesLite-II (HL2) specific options are shown in Fig. \ref{fig:RadioMenuHL2}.
...
\begin{figure}[ht]
\center
\includegraphics[scale=0.6]{RadioMenuHL2.png}
\caption{The \bltt{Radio} menu setting for a HermesLite-II.}
\label{fig:RadioMenuHL2}
\end{figure}
...
For these boards, there are normally no specific settings to be made
(Fig. \ref{fig:RadioMenuHermes}).

\begin{figure}[ht]
\center
\includegraphics[scale=0.6]{RadioMenuHermes.png}
\caption{The \bltt{Radio} menu setting for Hermes boards.}
\label{fig:RadioMenuHermes}
\end{figure}

\rett{Anan-10E/100B}. While the Anan-10E and
...
(Fig. \ref{fig:RadioMenuSoapy}).

\begin{figure}[ht]
\center
\includegraphics[scale=0.6]{RadioMenuSoapy.png}
\caption{The \bltt{Radio} menu setting for SoapySDR radios.}
\label{fig:RadioMenuSoapy}
\end{figure}
...
In the audio modules (for ALSA, PortAudio and PulseAudio/PipeWire),
...
and predictable pace, such that we do not need a large latency to cope with small pauses
...
\pH was compiled with the AUDIO=ALSA compile-time option (see Appendix \ref{sec:compiletime}.
With the ALSA audio module (see Fig. \ref{fig:SideToneALSA})
...
\pH. In my preferred setup, I use an external keyer built around a micro-controller, which
...
that result from the sharp rise and fall of the side tone pulse envelopes. Both problems are eliminated
...
Fig. \ref{fig:SideToneExternal} shows the measured result of such a setup. One immediately
