...
piHPSDR organises the transfer of digitised radio (IF) data between the radio hardware and the
WDSP library, the
...
dials that control piHPSDR. Apart from home-brew projects in which a micro-controller such as an Arduino
...
\item[Controller1]{Choose this if you have the original piHPSDR controller put to the market by ApacheLabs
...
since this radio has a programmable pre-amplifier/attenuator that goes from -12 to +48 dB. For the STEMlab
(Red Pitaya based) radios, you find a combo-box here where you can select the fixed attenuators/preamps.
...
level and want to disable squelch temporarily, you can un-check the check box and active squelch later
on by checking it.
...
at a larger font size. For example, if the standard VFO bar for a piHPSDR
...
top right corner of the piHPSDR window, the outcome is shown in Fig. \ref{fig:MainMenu}.

Some remarks have to be made about menus in general. Since piHPSDR is optimised for
...
the XX are heaxdecimal numbers that encode the MAC address for the radio.
...
\rett{VFO Encoder Divisor:} This option is visible only if using a GPIO controller.
...
15, only every 15$^\textrm{th}$ tick will we processed. In my view, one should rather decrease
the VFO step size than using a divisor value larger than one, but the option is left
...
\caption{Five choices for the VFO bar built into piHPSDR.}
...
if piHPSDR is to be run alongside with a logbook and/or digi mode program on a small laptop.
...
The \bltt{Display} menu is used to customise the overall layout of the piHPSDR
...
and equaliser settings, and TX compressor settings
...
RX equaliser & all settings \\\hline
TX equaliser & all settings \\\hline
...
artefacts, so there is an option to reduce such artefacts which should normally be checked
(artefact elimination ,,on'').
...
correction. This we can do since we do not allow low-latency filters in the TX
(see \bltt{DSP} menu).
...
\begin{figure}[ht]
\center
\includegraphics[width=8cm]{PAmenuCalibrate.png}
...
a 100W PEP signal. Non-linearities of the PA first lead to the occurrence of harmonics
...
(8 msec) is fine for CW speeds up to 32 wpm, you can choose here values from
5 to 16 msec (piHPSDR does not allow values smaller than 5 msec
to ensure that your CW signal does not produce overly strong clicks).
If the RF pulse is made
by piHPSDR (\rett{CW handled in Radio} \textit{not} checked),
this ,,ramp width change'' will be effective. A very recent Protocol-2 update
...
unless you have a very recent model, so with
...
a ramp with a constant width of 5 msec is used here.
...
to change this. Note that piHPSDR does not allow low latency
...
is hard-coded in piHPSDR to be 1024 (except for the transmitter in Protocol-2,
...
wants to change the equaliser settings for the receivers
...
By default, the equalisers are four-channel equalisers, but
you can individually change each of the three to a ten-band
one if you check the \rett{Ten Bands} box. If you do so, additional
spin buttons and sliders appear to the right of those already
seen (see Fig. \ref{fig:TenBandEqualizer}), such that the menu
window gets quite broad (about 900 pixels) so the use of ten-
band equalizers may become cumbersome for very small screens.
In normal situations, where the audio band width is usually 3 kHz
or less, a four-channel equaliser is usually sufficient. The possibility
to have (up to) ten channels can be useful for wide band (AM or eSSB) operation.

\begin{figure}[ht]
\center
\includegraphics[width=13cm]{TenBandEqualizer.png}
\caption{Broad version of the Equaliser menu (in 10-channel mode)}
\label{fig:TenBandEqualizer}
\end{figure}

The four (or ten) corner frequencies can be changed with the
spin buttons, while the corresponding gain/attenuation is
set with the slider to the right thereof. Between the corner frequencies, the
(frequency dependent!) gain/attenuation is obtained by linear interpolation.
The gain/attenuation specified for the lowest corner frequency
then applies to all frequencies below, and likewise for the
highest corner frequency.
The top left slider does not have a spin button but is
labeled \rett{Preamp},
which indicates that this slider applies an
additional, frequency-independent gain or attenuation.


Equaliser settings are saved with the mode, so
if you adjust the Equalisers when doing LSB, and then switch to
DIGU, the equalisers are disabled and they resume their LSB
...
selected. The corner frequencies cannot ,,cross'', that is,
the spin buttons are programmed such that
corner frequencies of the (four or ten) channels are kept
...
as long as you are TUNE-ing with piHPSDR. Besides the \bltt{TUNE} action,
...
In this chapter, the customisation of the toolbar (at the bottom of the piHPSDR window),
...
of, say, a logbook program running on the same computer to piHPSDR, even if the logbook program cannot use
...
\rett{Serial}. piHPSDR support up two two CAT connections over serial lines or named pipes.
In the text field, enter the device name of the serial port or the named pipe to use.
Which names to use
is highly operating system dependent. On a RaspPi, the built-in serial ports is usually referred to
by the device file \texttt{/dev/ttyS0}. USB-to-serial
...
\texttt{/dev/ttyACM0} or \texttt{/dev/USB0} (on RaspPi) or \texttt{/dev/tty.usbserial-....} (on MacOS).
 piHPSDR does not try to ,,detect'' serial
...
,,\texttt{mkdir -p <some\_arbitrary\_name>}''. Names pipes can be used for the communication between piHPSDR
and another program running on the same computer, and are only meaningful if that other programm does not
...
\rett{Enable}. This checkbox enables or disables the piHPSDR CAT subsystem for that particular case.
Enabling and disabling can be done separately for the TCP subsystem and each of the serial
connections.

\rett{Andromeda}. This checkbox enables the connection for use with ANDROMEDA consoles. Serial lines are
forced to 9600 baud if enabled for ANDROMEDA. Furthermore, status changes of piHPSDR (e.g. RIT on/off) are
automatically reported via ZZZI CAT commands (see Appendix \ref{sec:catcommands}). These messages are
used by the ANDROMEDA console to switch LED indicators such that they reflect the current state 
of piHPSDR. Note that this flag is only inspected when the connection is opened. Serial CAT connections
...
\rett{Enable RigCtl Debug Logging}. If enabled, the piHPSDR CAT subsystem sends lots of debug messages to the standard output. If
piHPSDR is run
...
\action{AF Gain}{AFGAIN}{PE}{Change the AF gain (headphone volume, $-40\ldots 0$ dB) of the active receiver.}

\action{AF Gain RX1}{AFGAIN1}{PE}{Change the AF gain (headphone volume, $-40\ldots 0$ dB) of the RX1 receiver.}

\action{AF Gain RX2}{AFGAIN2}{PE}{Change the AF gain (headphone volume, $-40\ldots 0$ dB) of the RX2 receiver. This operation is
a no-op if only one receiver is running.}

\action{AGC}{AGCT}{B}{Cycle through the AGC modes (slow, medium, fast, $\ldots$) of the current receiver.}

\action{AGC Gain}{AGCGain}{PE}{Change AGC gain ($-20\ldots 120$ dB) of the active receiver.}

\action{AGC Gain RX1}{AGCGain1}{PE}{Change AGC gain ($-20\ldots 120$ dB) of RX1.}

\action{AGC Gain RX2}{AGCGain1}{PE}{Change AGC gain ($-20\ldots 120$ dB) of RX2. This is a no-op if
only one receiver is running.}

\action{AGC Menu}{AGC}{B}{Opens the \texttt{AGC} menu.}

\action{ANF}{ANF}{B}{Toggles the state (on/off) of the automatic notch filter for the active receiver.}

\action{Atten}{ATTEN}{PE}{Changes the value (0-31 dB) of the step attenuator of the active receiver.
This function is only available for radios that have such an attenuator.}
...
{\bf Note that the RX equalisers are switched off during recording, and the TX equaliser and compressor
is switched off during playback.}
The volume of the recorded audio is normalised by applying an appropriate scale
factor to the entire recording (this is a linear operation, in contrast to compression).
This also implies that during playback, the mic gain is temporarily set to unity (0 dB).
}
...
\action{CW Key (keyer)}{CWKy}{B}{Straight key key-down or key-up event. Usually assigned to a GPIO line of
MIDI controller to
...
\action{Mute RX1}{MUTE1}{B}{Toggles the ,,mute'' state of  RX1. If a receiver is muted, it
produces zero-amplitude audio output.}

\action{Mute RX2}{MUTE2}{B}{Toggles the ,,mute'' state of  RX2. This operation is a no-op
if only one receiver is running. If a receiver is muted, it
produces zero-amplitude audio output.}
...
\action{NumPad BS}{BS}{B}{Used for direct frequency entry (BS = back-step). This is the same as hitting the
...
\action{Panadapter Step}{PANS}{PE}{Change the step size (from 5 to 30) of the pan-adapter of the active
...
\action{PS On/Off}{PST}{B}{Toggle (on/off) adaptive pre-distortion (PureSignal).}
...
a microphone to
a GPIO line for this purpose, take care of proper debouncing, since piHPSDR is not good at debouncing
switches where both
...
\action{SNB}{SNB}{B}{Toggle (enable/disable) the spectral noise blanker for the active  receiver.}
...
\action{Tune Drv}{TUNDRV}{E}{Change the drive  level (0-100) used for TUNE-ing. This is equivalent to
...
\action{VFO Step -}{STEP-}{E}{Cycle downwards through the VFO step sizes of the VFO
controlling the active receiver. If the step size was already the smallest one,
jump to the largest.}

\action{VFO Step +}{STEP+}{E}{Cycle upwards through the VFO step sizes of the VFO
...
A small subset of the extended commands  has been implemented in piHPSDR. This set has later been extended
...
$\phantom{\Big|}${\large Notes} & \multicolumn{1}{|p{11cm}|}{When setting (sending), p$_1$ must be a space.} \\
...
$\phantom{\Big|}${\large Notes} & \multicolumn{1}{|p{11cm}|}{When setting, any nonzero p$_1$ sets lock status} \\
...
$\phantom{\Big|}${\large Notes} & \multicolumn{1}{|p{11cm}|}{Kenwood-type  mode  list:} \\
...
$\phantom{\Big|}${\large Notes} & \multicolumn{1}{|p{11cm}|}{p$_1$ = 0 ... 99 is mapped to the range available} \\
...
$\phantom{\Big|}${\large Notes} & \multicolumn{1}{|p{11cm}|}{p$_1$=0: neither SAT nor RSAT, p$_1$=1: SAT or RSAT} \\
 ...
 & \multicolumn{1}{|p{11cm}|}{when setting, p$_4$ is ignored} \\
\bottomrule
\end{tabular}
\end{center}
...
$\phantom{\Big|}${\large Notes} & \multicolumn{1}{|p{11cm}|}{Automatic generated response for ANDROMEDA controller.} \\
 & \multicolumn{1}{|p{11cm}|}{The LED with number p$_1$ shall be switched on (p$_2$=1) or off (p$_2$=0).} \\
\bottomrule
\end{tabular}
\end{center}
...
$\phantom{\Big|}${\large Notes} & \multicolumn{1}{|p{11cm}|}{ANDROMEDA extension.} \\
 & \multicolumn{1}{|p{11cm}|}{The ANDROMEDA type (p$_1$), hardware (p$_2$) and} \\
 & \multicolumn{1}{|p{11cm}|}{software (p$_3$) version is printed in the log file.} \\
 & \multicolumn{1}{|p{11cm}|}{The type (p$_1$) sent by a client (currently, 1 or 5)} \\
 & \multicolumn{1}{|p{11cm}|}{does affect the processing of ZZZE and ZZZP} \\
 & \multicolumn{1}{|p{11cm}|}{commands from that client.} \\
 & \multicolumn{1}{|p{11cm}|}{} \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{MIDI.}
If running piHPSDR on non-RaspPi Linux or Macintosh computers, there are no GPIO
lines. The same problem arises for piHPDSR running on a RaspPi if using the
Controller2 or the G2 front panel, simply because nearly all GPIO lines are
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter[piHPSDR and digi mode programs]{Running piHPSDR alongside with DigiMode programs}
...
\item{piHPSDR and digi mode program running on the same LINUX computer
...
\item{piHPSDR and digi mode program running on the same MacOS computer.}
\end{itemize}
...
screen, such as it is the case if you are using a piHPSDR controller or the G2 front panel.
...
and piHPSDR. Via CAT commands, the digi mode program can induce RX/TX transitions in piHPSDR,
...
can be changed in the \bltt{RigCtl} menu). For standard digi mode operation, choose the
...
If there is no ethernet connection between the computer running piHPSDR and the computer
...
as digital data between piHPSDR and the digi mode program. To this end, you need
...
compilation of piHPSDR. If compiling from the sources you get the PulseAudio module if you
...
Then you can change the file \texttt{GNUmakefile} and recompile the program (see end of this section!).
...
within piHPSDR, for example since you have a controller (Controller1, Controller2, of G2 front panel)
...
This option activates the possibility to control piHPSDR via MIDI devices. It should normally be
...
%%\end{center}
%%Binary installation means that the executable program has been compiled and linked on my
...
In the last year, MacOS operating systems from version 10.15 (,,Catalina'') through 14.5 (,,Sonoma'')
have been used. Very old MacOS systems (e.g. Catalina) are no longer supported by HomeBrew.}

If you want to run piHPDSR on an Apple Macintosh computer (iMac, Mac Mini, iBook Air, PowerBook), then
you \textit{have to} compile piHPSDR from the sources. Producing a pre-compiled binary that can be
distributed and run simply by double-clicking is overly complicated for applications using the GTK
graphical user interface (as piHSPDR does). But fear not, the proceduce is actually very simple because
...
\label{sec:dhcp}
\textit{This section applies if you run piHPSDR on a
RaspPi or aDesktop or Laptop computer running Linux or MacOS, and if this computer connects to the
...
\textit{This section further does \textbf{not} apply to the new Saturn/G2 radios when running piHPSDR on the
...
To make the communication work, the easiest workaraound is probably to assign a fixed IP address both to
...
subnet. That is, the so-called netmask is 255.255.255.0, alternatively, the addresses are given in the
form $192.168.xxx.yyy/24$ where the number 24 encodes the netmask.

Such local subnets with small numbers of $xxx$ (say, 1 through 4) are often created/used by routers etc.,
so it is strongly recommended to use a higher number, e.g. $xxx=8$ for the subnet used for connecting
the comnputer and the radio.
...
In principle, one can have a DHCP server running on the computer that also runs piHPSDR, and thi8
