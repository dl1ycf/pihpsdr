...
\textbf{Antenna Tuner.} When starting TUNE-ing, a '1' is written into register 7, and when ending TUNE-ing, a '0'
is written to that register. The Pico then must take care to start/stop a TUNE-ing sequence.
Writing a '0' at the end of TUNE is not necessary when everything went OK, but may be needed
to stop the auto-tuner from searching a match when the user decided to end the TUNE-ing.

\textbf{PURE feedback input.} The RF input path determines how the RF input jacks are connected to the RF front-end.
This is automatically done depending on whether PURESIGNAL is used and whether we are
transmitting or receiving. When using PURESIGNAL, connect the RF feedback signal to
the input jack named PURE.
