...
The audio latency is of utmost importance when doing CW, especially
if the iambic keyer built into \pH is used: imagine you want to sent a letter 'V' in CW,
...
an audio delay of more than 100 ms is certainly not acceptable when doing CW. On the other hand,
the side tone audio data can be produced at a much more predictable pace than the RX audio, since
it involves no DSP processing. Therefore, it is safe to reduce the audio latency drastically when
transmitting in CW. \pH does this on MacOS, and on LINUX when using the ALSA audio module. In these
cases, a ,,rewind'' operation is made on the audio output stream, that is, a lot of audio samples
from the output buffer are discarded such that the buffer is at a rather low filling. By making the
pauses between dots and dashes very slighly larger or smaller, depending on the buffer filling,
\pH manages to keep the buffer filling constant at a rather low level, this minimising the
audio delay.
