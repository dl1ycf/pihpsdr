...
The next line with check boxes \rett{Split}, \rett{Duplex}, \rett{Mute RX on TX} and
\rett{PA enable} is only shown for radios that have a transmitter.
...
We first go through the controls in the upper half of the noise menu:
...
It should also be noted that noise reduction in general has two
...
suits your needs. Again, changing the settings for NR2 requires that you have read
(and understood) the discussion of NR2 in the WDSP manual. The noise menu with the NR2-specific
control is shown in Fig. \ref{fig:NoiseMenu2}:
...
delay (latency) is of interest but also to which extent the RX/TX transition "swallows" parts of the
first dashes sent.}
\item[TX/RX turnover]
...
are discarded such that the latency is small again. Furthermore, \pH very slightly
